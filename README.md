...
- **[PERBARUI PASCA-EKSPERIMEN] vCPU = hyperthread, bukan physical core penuh — TIDAK RELEVAN pada konfigurasi final.** Poin ini semula menjelaskan Option 1 (`c2-standard-8`, SMT aktif, 8 vCPU = 4 physical core, kontaminasi sibling reserved-CPU vs solver-CPU tak terhindarkan). Karakterisasi topologi (`characterize_topology.py`) mengarahkan eskalasi ke **Option 3** (`c2-standard-16`, SMT nonaktif via `--threads-per-core=1`, 8 physical core independen, verified via `lscpu`: `Thread(s) per core: 1`) sebagai konfigurasi yang benar-benar dipakai untuk pengumpulan 300-run. Pada Option 3, tidak ada pasangan sibling sama sekali, sehingga kontaminasi sibling yang dideskripsikan di atas **tidak terjadi** pada data yang dilaporkan. Baris ini sebelumnya tidak diperbarui setelah eskalasi topologi dan bisa membingungkan pembaca kode yang mengira Option 1 adalah konfigurasi final — lihat Bab Metode Subbab 3.2.2 skripsi untuk kronologi lengkap dan hasil `lscpu` yang sebenarnya dipakai.
...
- **Isolasi core Kubernetes ≠ isolasi kernel penuh.** `reservedSystemCPUs` membatasi scheduler decisions, bukan timer interrupt atau softirq yang tetap bisa membebani core "eksklusif". Ini merepresentasikan skenario cloud-native nyata di mana peneliti tidak memiliki akses ke `isolcpus`/`nohz_full`. Lebih lanjut, Kubernetes API secara desain menolak konfigurasi `reservedSystemCPUs` jika `cpuManagerPolicy` di-set ke `none` (seperti pada Kondisi A). Oleh karena itu, pada Kondisi A, isolasi semata-mata bergantung pada Cgroup weights (`kubeReserved` dan `systemReserved`), bukan pada exclusive cpuset. Ketiadaan `reservedSystemCPUs` di file config Kondisi A adalah kebutuhan teknis sistemik, bukan omission bug.
...
- **Isolasi I/O Storage.** Seluruh operasi penulisan hasil akhir ke volume host (`results-host` yang merupakan `hostPath` disk persisten) dieksekusi secara ketat *setelah* pemanggilan `model.dispose()` dan setelah keseluruhan waktu penyelesaian solver tercatat. Segala bentuk I/O yang berinteraksi dengan instrumen performa Gurobi selama eksperimen (barrier atau crossover window) menggunakan memory-backed filesystem (`emptyDir: Memory`) untuk mencegah disk I/O jitter.
...
- **Cakupan Stabilitas Barrier (RQ3) terbatas.** Metrik `barrier_iteration_seconds` yang diukur secara operasional mengecualikan waktu presolve/startup Gurobi sebelum iterasi pertama barrier berjalan, merepresentasikan durasi pure phase, namun tidak end-to-end proses Gurobi.
- **Auto-Resume dan noise temporal.** Mekanisme *5x retry wrapper* (`run_experiment.sh`) menjamin keberhasilan target repetisi (N=30) di lingkungan yang fluktuatif seperti komputasi awan. Tetapi restart proses otomatis dapat menginduksi cache flushes/timing gaps yang dapat memengaruhi presisi durasi temporal repetisi berikutnya. Hal ini didokumentasikan untuk transparansi reproducibility.
